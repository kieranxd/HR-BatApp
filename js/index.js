...
const spectrogramBorder = document.getElementById(
  "spectrogram-resizable-border-div"
);
const spectrogramHeterodyneSVG = document.getElementById(
  "spectrogram-heterodyne-svg"
);
...
const intervalButton = document.getElementById("capture-wav-button");
...
const displayWidthPagination = document.getElementById(
  "display-width-pagination"
);
const displayWidthValues = [1, 5, 10, 20, 60];
const autosaveLengthPagination = document.getElementById(
  "autosave-length-pagination"
);
const autosaveLengthValues = [1, 5, 10, 60];
const monitorPagination = document.getElementById("monitor-pagination");
const monitorValues = [0, 1, 2]; // Off, Monitor, Heterodyne

let CURRENTAUTOSAVE = 1;
...
const errorDontShowCheckbox = document.getElementById(
  "error-dont-show-checkbox"
);
...
electron.ipcRenderer.on("poll-night-mode", () => {
  electron.ipcRenderer.send("night-mode-poll-reply", true);
});
...
function sendSimulationInfo() {
  const simulationPath = app.isPackaged
...
electron.ipcRenderer.on("simulate", (e, index) => {
  if (monitorMode === 0 || index !== -1) {
    ...
    const buttonText =
      monitorMode === 1 ? "Disable Monitor" : "Disable Heterodyne";

    twoOption.displayTwoOptionCheck(
      "Warning",
      "Make sure your microphone is positioned away from your speakers to prevent audio feedback.",
      "Continue",
      () => {
        changeSimulation(index);
      },
...
function endResize() {
  ...
  const width = Math.min(
    waveformBorder.offsetWidth,
    spectrogramBorder.offsetWidth
  );

  plotter.resize(
    width,
    waveformBorder.offsetHeight,
    spectrogramBorder.offsetHeight
  );
...
function isHeterodyneEnabled() {
  return (
    paginationButtons.getActiveValue(monitorPagination, monitorValues) === 2
  );
}
...
function isMonitorEnabled() {
  return (
    paginationButtons.getActiveValue(monitorPagination, monitorValues) === 1
  );
}
...
function checkHeterodyneButtons() {
  heterodyneUpButton.disabled =
    heterodyneFrequency + 100 > heterodyneFrequencyMax;

  heterodyneDownButton.disabled =
    heterodyneFrequency - 100 < heterodyneFrequencyMin;
}
...
function updateHeterodyneUI(reset) {
  ...
  heterodyneFrequency =
    reset || sampleRate < 48000 ? sampleRate / 4 : heterodyneFrequency;
...
function updateDisplay() {
  ...
  if (result.redrawRequired) {
    ...
    if (
      !oldDeviceName.includes("Simulated") &&
      !newDeviceName.includes("Simulated")
    ) {
      if (
        (oldDeviceName.includes("Default Input") &&
          newDeviceName.includes("AudioMoth USB Microphone")) ||
        (newDeviceName.includes("Default Input") &&
          oldDeviceName.includes("AudioMoth USB Microphone"))
      ) {
        if (isMonitorEnabled() || isHeterodyneEnabled()) {
          console.log(
            "Switched between default input and AudioMoth USB Mic, disabling monitor/heterodyne"
          );
  ...
  if (!resizing)
    plotter.update(
      ...
      result.audioIndex,
      result.audioCount,
      displayWidth * currentSampleRate,
      nightMode.isEnabled(),
      lowAmpColourScaleEnabled
    );
  ...
  const intervalToNextFrame =
    FRAME_INTERVAL * Math.ceil(milliseconds / FRAME_INTERVAL) - milliseconds;
...
function startMicrophone() {
  ...
  if (!result.success) {
    displayError(
      ...
      "Could not initialise the input device."
    );
  }
  ...
  backstage.setAutoSaveCallback(() => {
    ...
    if (!displayingAutosaveWarning) {
      ...
      displayError(
        "Autosave error",
        "Failed to save a WAV file. Check the file destination is still valid.",
        false,
        false,
        () => {
...
const inputDir = path.join(__dirname, "input", "video");
...
const processedDir = path.join(__dirname, "processed", "video");
...
function analyseVideo() {
  ...
  const output =
    "--output=" +
    `"${path.join(outputDirData, `video_data_${currentDateTime}.csv`)}"`;

  const args = [input, output, "--saveboxes"];

  const batProcess = spawn(`"${batFilePath}"`, args, {
    encoding: "utf-8",
    shell: true,
  });
  ...
  batProcess.on("close", (code) => {
    const csvFilePath = path.join(
      outputDirData,
      `video_data_${currentDateTime}.csv`
    );
    ...
    files.forEach((file) => {
      ...
      fs.rename(oldPath, newPath, (err) => {
        if (err) {
          ...
          console.log(`Moved file ${file} to processed directory`);
        }
      });
    });
  });
...
function analyseAudio() {
  ...
  const shell = spawn(`py "${pythonScript}"`, {
    shell: true,
    cwd: path.join(__dirname, "batdetect2"),
  });
...
/**
 * Start the interval for capturing audio and run the deep learning models
 */
function interval() {
  ...
  function setAutosave() {
    ...
    if (CURRENTAUTOSAVE === 0) {
      ...
      backstage.setAutoSave(CURRENTAUTOSAVE);
...
function setPause(newPause) {
  ...
  if (pauseResult.audioIndex !== undefined) {
    ...
    exportPlotter.prepare(
      audioBuffer,
      stftBuffer,
      pauseResult.audioIndex,
      pauseResult.audioCount,
      displayWidth,
      currentSampleRate,
      lowAmpColourScaleEnabled
    );
...
function handleMonitorChange(monitorSelection) {
  if (monitorSelection === 1) {
    ...
    console.log(
      "Heterodyne enabled with frequency:",
      heterodyneFrequency,
      "Hz"
    );
    changeAndSaveMonitorMode(backstage.MONITOR_HETERODYNE);
...
paginationButtons.initButtons(monitorPagination, (activeIndex) => {
  ...
  if (monitorSelection === 0) {
    ...
    if (simulationRunning) {
      ...
      const buttonText =
        monitorSelection === 1 ? "Disable Monitor" : "Disable Heterodyne";
